...
static std::string renderVertexShaderPath = "C:\\Users\\y123456\\Desktop\\Programming\\c_cpp\\GameEngine\\Resources\\shaders\\cubeRender.vert";
static std::string renderFragmentShaderPath = "C:\\Users\\y123456\\Desktop\\Programming\\c_cpp\\GameEngine\\Resources\\shaders\\cubeRender.frag";
...
void VulkanCubeMapRender::init(
                               const VulkanTexture& inputTexture,
                               const VulkanTexture& targetTexture) {
    //  VulkanTexture* inputHDR= nullptr;
    renderVertex.compileFromFile(renderVertexShaderPath,ShaderKind::vertex);
    renderFrag.compileFromFile(renderFragmentShaderPath,ShaderKind::fragment);
    renderQuad.createQuad(2.0f);
    ...
    VulkanGraphicsPipelineBuilder pipelineBuilder(context,pipelineLayout,renderPass);
    pipeline = pipelineBuilder
            .addShaderStage(renderVertex.getShaderModule(), VK_SHADER_STAGE_VERTEX_BIT)
            .addShaderStage(renderFrag.getShaderModule(), VK_SHADER_STAGE_FRAGMENT_BIT)
            .addVertexInput(VulkanMesh::getVertexInputBindingDescription(), VulkanMesh::getAttributeDescriptions())
            .setInputAssemblyState(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST)
            .addViewport(viewport)
            .addScissor(scissor)
