...
VulkanGraphicsPipelineBuilder::addShaderStage(VkShaderModule shader, VkShaderStageFlagBits stage, const char *entry) {
    ...
    vertexShaderStageInfo.module = shader;
    vertexShaderStageInfo.stage = stage;
    vertexShaderStageInfo.pName =entry;
    shaderStages.emplace_back(vertexShaderStageInfo);
    return *this;
}

VulkanGraphicsPipelineBuilder &
VulkanGraphicsPipelineBuilder::addVertexInput(const VkVertexInputBindingDescription &binding,
...
VulkanGraphicsPipelineBuilder &
VulkanGraphicsPipelineBuilder::addBlendColorAttachment(bool blend, VkBlendFactor srcColorBlendFactor,
...
VulkanGraphicsPipelineBuilder &
VulkanGraphicsPipelineBuilder::addDescriptorSetLayoutBinding(uint32_t binding, VkDescriptorType type,
...
VulkanGraphicsPipelineBuilder &
VulkanGraphicsPipelineBuilder::addColorAttachment(VkFormat format, VkSampleCountFlagBits msaaSamples) {
    // Create render pass
    ...
    colorAttachmentResolveReferences.emplace_back(colorAttachmentResolveReference);
    return *this;

}

VulkanGraphicsPipelineBuilder &
VulkanGraphicsPipelineBuilder::addDepthStencilAttachment(VkFormat format, VkSampleCountFlagBits msaaSamples) {
...
VulkanGraphicsPipelineBuilder &
VulkanGraphicsPipelineBuilder::setInputAssemblyState(VkPrimitiveTopology topology, bool primitiveRestart) {
    inputAssemblyState = {};
...
VulkanGraphicsPipelineBuilder &
VulkanGraphicsPipelineBuilder::setRasterizerState(bool depthClamp, bool rasterizerDiscard, VkPolygonMode polygonMode,
                                                  float lineWidth, VkCullModeFlags cullMode, VkFrontFace frontFace,
                                                  bool depthBias, float depthBiasConstantFactor, float depthBiasClamp,
...
VulkanGraphicsPipelineBuilder &
VulkanGraphicsPipelineBuilder::setMultisampleState(VkSampleCountFlagBits msaaSamples, bool sampleShading,
...
VulkanGraphicsPipelineBuilder &
VulkanGraphicsPipelineBuilder::setDepthStencilState(bool depthTest, bool depthWrite, VkCompareOp depthCompareOp) {
...
void VulkanGraphicsPipelineBuilder::build() {
    ...
    if (vkCreateDescriptorSetLayout(context.device_, &descriptorSetLayoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS)
    ...
    if (vkCreateGraphicsPipelines(context.device_, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline) != VK_SUCCESS)
