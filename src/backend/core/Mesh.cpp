...
bool import(const char *file, std::vector<Vertex> &vertices, std::vector<uint32_t> &indices)
{
    ...
    return import(scene->mMeshes[0],vertices,indices);
}
...
bool import(const aiMesh *mesh, std::vector<Vertex> &vertices, std::vector<uint32_t> &indices)
{
    ...
    for (uint32_t i = 0; i < mesh->mNumFaces; i++) {
        for (uint32_t faceIndex = 0; faceIndex < meshFaces[i].mNumIndices; faceIndex++) {
...
bool import_scene(const char* file)
{
    Assimp::Importer importer;
    unsigned int flags = aiProcess_GenSmoothNormals |
        aiProcess_CalcTangentSpace |
