...
)
{
    ...
    if (vkCreateShaderModule(device->LogicDevice(), &shaderInfo, nullptr, &shader) != VK_SUCCESS)
...
)
{
    ...
    if (vkCreateImageView(device->LogicDevice(), &viewInfo, nullptr, &imageView) != VK_SUCCESS)
        return VK_NULL_HANDLE;
...
)
{
    ...
    if (vkCreateSampler(device->LogicDevice(), &samplerInfo, nullptr, &sampler) != VK_SUCCESS)
        return VK_NULL_HANDLE;
...
VkCommandBuffer Utils::beginSingleTimeCommands(const render::backend::vulkan::Device *device)
{
    ...
    vkAllocateCommandBuffers(device->LogicDevice(), &allocInfo, &commandBuffer);
...
void Utils::endSingleTimeCommands(const Device *device, VkCommandBuffer commandBuffer)
{
    ...
    VkFence fence;
    if (vkCreateFence(device->LogicDevice(), &fenceInfo, nullptr, &fence) != VK_SUCCESS)
    ...
    if (vkWaitForFences(device->LogicDevice(), 1, &fence, VK_TRUE, UINT64_MAX) != VK_SUCCESS)
        throw std::runtime_error("Can't wait for a fence");

    vkDestroyFence(device->LogicDevice(), fence, nullptr);

    vkFreeCommandBuffers(device->LogicDevice(), device->CommandPool(), 1, &commandBuffer);
}
