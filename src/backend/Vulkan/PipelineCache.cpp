...
uint64_t PipelineCache::getHash(VkPipelineLayout layout,const context *ctx, const RenderPrimitive *primitive) const
{
    ...
    auto* vertex_buffer = primitive->vertexBuffer;
    for (int i = 0; i <primitive->vertexBuffer->num_attributes; ++i) {
...
VkPipeline PipelineCache::fetch(const context *ctx, const RenderPrimitive *render_primitive)
{
    ...
    auto* vertex_buffer = render_primitive->vertexBuffer;
    VkVertexInputBindingDescription vertex_input_binding{0,vertex_buffer->vertex_size};
    ...
    builder.setInputAssemblyState(render_primitive->topology);
