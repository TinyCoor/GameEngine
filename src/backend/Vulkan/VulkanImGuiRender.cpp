...
void ImGuiRender::init(ImGuiContext *context)
{
...
void ImGuiRender::render(const VulkanRenderFrame& frame)
{
    ...
    primitive.vertex_buffer = vertices;
    primitive.index_buffer = indices;
...
void ImGuiRender::updateBuffers(const ImDrawData *draw_data)
{
...
void ImGuiRender::setupRenderState(const vulkan::VulkanRenderFrame &frame, const ImDrawData *draw_data)
{
    ...
    driver->setPushConstant(size, transform);

    driver->setBlending(true);
    driver->setBlendFactor(BlendFactor::SRC_ALPHA, BlendFactor::ONE_MINUS_SRC_ALPHA);
...
/*
 */
ImTextureID ImGuiRender::fetchTextureID(const backend::Texture *texture)
{
    auto it = registered_textures.find(texture);
    if (it != registered_textures.end())
        return it->second;

    BindSet *bind_set = static_cast<BindSet *>(driver->createBindSet());
...
void ImGuiRender::invalidateTextureIDs()
{
